...
export async function findGeoCandidates(name, { stateName = null } = {}) {
  ...
  // Include a state candidate when the target exactly matches a state name.
  // States get a 10× boost so they decisively outrank any same-name city/county.
  const stateCandidate = lookupStateByName(target);
  ...
  // Sort by population-weighted score. States get a 10× boost so they rank above
  // any same-name city when the search term is a state name. Places get 5× so the
  // intended city ranks above a same-name metro; subdivisions get 0.2× because they're
  // rarely the intended target. Large metros still float above tiny villages.
  const TYPE_BOOST = { state: 10, place: 5, county: 2, cbsa: 1, urban_area: 1, county_subdivision: 0.2 };
  merged.sort((a, b) => {
...
export function describeCandidate(c) {
  ...
  switch (c.geoType) {
    case "state":
      return {
        label: c.name,
        sublabel: `State · pop ${fmtPop(c.population)}`,
        icon: "",
      };
    case "place":
      return {
