...
// The metric phrase and the location are split on the FIRST of these. Order
// in the array doesn't matter — the earliest match in the query wins.
const LOCATION_SEPARATORS = [" in ", " of ", " for "];

// First location separator in the (lowercased) query → { index, length },
// or { index: -1 } when the query contains none.
function findLocationSeparator(q) {
