...
export async function fetchCensusValueWithMOE(variableId, geoParams, apiKey, year = DEFAULT_YEAR, dataset = "acs/acs5") {
  ...
  if (placeFilter) {
    ...
    if (!row && forGeo === "place:*") {
      ...
      const subParams = new URLSearchParams({ get: `NAME,${variableId},${moeId}`, for: "county subdivision:*", key: apiKey });
      if (inGeo) subParams.set("in", inGeo);
      const subRes = await fetch(`${BASE_URL_BASE}/${year}/${dataset}?${subParams.toString()}`);
      if (subRes.ok) {
        const subData = await subRes.json();
        if (Array.isArray(subData) && subData.length >= 2) {
          row = subData.slice(1).find(r => matchesCityName(r[0], filter));
          if (row) {
            // Re-derive indices from subData header
            const subHeader = subData[0];
            const subValueIdx = subHeader.indexOf(variableId);
            const subMoeIdx = subHeader.indexOf(moeId);
            return {
              value: subValueIdx === -1 ? null : row[subValueIdx],
              moe: subMoeIdx === -1 ? null : row[subMoeIdx],
            };
          }
        }
      }
...
export async function fetchCensusValue(variableId, geoParams, apiKey, year = DEFAULT_YEAR, dataset = "acs/acs5") {
  ...
  if (placeFilter) {
    ...
    if (!match) {
      // Fallback: try county subdivision geography (e.g. New England towns)
      if (forGeo === "place:*") {
        const subParams = new URLSearchParams({ get: `NAME,${variableId}`, for: "county subdivision:*", key: apiKey });
        if (inGeo) subParams.set("in", inGeo);
        const subRes = await fetch(`${BASE_URL_BASE}/${year}/${dataset}?${subParams.toString()}`);
        if (subRes.ok) {
          const subData = await subRes.json();
          if (Array.isArray(subData) && subData.length >= 2) {
            const subMatch = subData.slice(1).find(row => matchesCityName(row[0], filter));
