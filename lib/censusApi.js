...
// Extract the actual FIPS values from a matched Census API response row so
// callers can build geo-filtered source URLs pointing at specific places instead
// of wildcards. Falls back to the original geoParams if the row has no FIPS cols.
function resolveGeoParamsFromRow(header, row, fallbackGeoParams) {
  const get = col => { const i = header.indexOf(col); return i !== -1 ? row[i] : null; };
  const stateVal = get("state");
  const placeVal = get("place");
  const countyVal = get("county");
  if (stateVal && placeVal)  return { forGeo: `place:${placeVal}`,  inGeo: `state:${stateVal}` };
  if (stateVal && countyVal) return { forGeo: `county:${countyVal}`, inGeo: `state:${stateVal}` };
  if (stateVal)              return { forGeo: `state:${stateVal}` };
  return fallbackGeoParams;
}
...
// England towns) in one place. Returns { value, moe, resolvedGeoParams } —
// moe is null when not requested or the variable has no simple E→M companion;
// resolvedGeoParams replaces wildcards with the actual matched FIPS.
async function fetchCensusRow(variableId, geoParams, apiKey, year, dataset, { withMOE }) {
  ...
  // response header so column order doesn't matter. Also extracts the actual
  // FIPS from the row so callers can build precise source-link URLs.
  const extract = (header, row) => {
    ...
    return {
      ...
      resolvedGeoParams: resolveGeoParamsFromRow(header, row, geoParams),
    };
  };
