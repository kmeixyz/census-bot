// lib/trend.js
// Core multi-year ACS trend computation, extracted from pages/api/trend.js so
// it can be called directly (in-process) by both the /api/trend HTTP route and
// the chat route's trend tool — no self-HTTP round-trip.
//
// `runTrend(input)` returns { status, body } where body is the JSON payload.
// On success: status 200 and body = { points, seriesWarning?, locationLabel,
// geoParams, unit, variableId, variableLabel, tableId }. On failure: a 400/500
// status with body = { error, ... }.

import { fetchCensusVariable } from "./censusApi";
import { parseQuery, VARIABLE_MAP, STATE_FIPS } from "./censusTranslator";
import {
  findGeoCandidates,
  findZctaByZip,
...
import { computeRateIfNeeded, hasRateConfig } from "./censusRates";
import { validateValue, detectAnomalies } from "./validateCensusData";
import { validateVariableClaim } from "./acsVariableMetadata";
import { findRedesignInRange, getTableChanges } from "./acsTableChanges";
import { findEquivalentVariableInYear, getVariableLeafLabel } from "./acsTablesRag";
...
export function isValidTrendYear(value) {
...
// Compute a multi-year ACS trend. Pure logic — no req/res. Returns
// { status, body } so HTTP and in-process callers map the result the same way.
export async function runTrend(input = {}) {
  const {
    city,
    state,
    location,
  ...
  if (!isValidTrendYear(startYear) || !isValidTrendYear(endYear)) {
    return { status: 400, body: { error: "startYear and endYear must be valid ACS years." } };
  }
  if (startYear > endYear) {
    return { status: 400, body: { error: "startYear must be less than or equal to endYear." } };
  }
  ...
  if (varResult.error) {
    return { status: 400, body: { error: varResult.error } };
  }
  ...
  if (cleanShareOf && !VARIABLE_ID_RE.test(cleanShareOf)) {
    return { status: 400, body: { error: `Invalid share_of_variable_id "${cleanShareOf}".` } };
  }
  ...
  if (points.length === 0 || points.every((p) => p.numericValue == null)) {
    return {
      status: 500,
      body: { error: `No Census data found for "${locationLabel}" in the requested year range.` },
    };
  }
